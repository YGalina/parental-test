...
<body>
  ...
  "difficultyVsUnsafety": '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>'
    ...
    function home() {
      ...
      const archetypeShowcase = '<section class="section" id="archetypes" style="padding-bottom:8px"><div class="section-inner"><div class="section-header fade-up"><div><p class="kicker" style="color:var(--pink)">шесть архетипов</p><h2>Каким родителем вы становитесь под давлением</h2></div><p style="max-width:420px">У каждого архетипа своя сила и своя слепая зона. Тест покажет ваш ведущий — и второй, если они близки.</p></div></div><div class="archetype-showcase stagger">' + ['director','anchor','mentor','guardian','partner','peacemaker'].map(k => { const a = ARCHETYPES[k]; return '<div class="arch-card"><div class="arch-card-img"><img src="' + a.image + '" alt="' + escapeHtml(a.name) + '" loading="lazy" /></div><h3>' + escapeHtml(a.name) + '</h3><p>' + escapeHtml(a.tagline) + '</p></div>'; }).join('') + '</div></section><hr class="divider">';
